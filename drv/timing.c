...
void expect_frequency(ClockSignalID signal, ClockFrequency freq)
{
	if(signal == CS_ACLK) while(CS_getACLK() != freq) {MAP_PCM_gotoLPM4();}
	else if(signal == CS_MCLK) while(CS_getMCLK() != freq) {MAP_PCM_gotoLPM4();}
	else if(signal == CS_SMCLK) while(CS_getSMCLK() != freq) {MAP_PCM_gotoLPM4();}
	else if(signal == CS_HSMCLK) while(CS_getHSMCLK() != freq) {MAP_PCM_gotoLPM4();}
	else if(signal == CS_BCLK) while(CS_getBCLK() != freq) {MAP_PCM_gotoLPM4();}
	else while(true) {MAP_PCM_gotoLPM4();}
}
